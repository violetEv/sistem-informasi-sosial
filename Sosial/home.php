...
<body>
  ...
  <div class="container text-center pt-4">
    <div class="row">
      <div class="col-md-6 text-md-start">
        <h2 class="display-6 fw-bold">Artikel</h2>
      </div>
        <?php if ($_SESSION['level'] == 'petugas') { ?>
            <div class="col-md-6 text-md-end">
                <a href="form-artikel.php" class="btn btn-primary">Manage Artikel</a>
            </div>
        <?php } ?>
    </div>
  </div>
  ...
  <!-- Display existing data for Kategori Bansos -->
  <div class="container-fluid existing-data">
    <div class="container">
        <h2 class="display-6 fw-bold">Data Artikel Kategori Bansos</h2>
        <div class="row">
            <?php 
            // Fetch existing data from the database for Kategori Bansos
            $query_bansos = "SELECT * FROM artikel WHERE kategori = 'bansos'";
            $result_bansos = $koneksi->query($query_bansos);
            while ($row_bansos = $result_bansos->fetch_assoc()) { ?>
                <div class="col-md-4 mb-4">
                    <div class="card">
                        <img src="<?php echo $row_bansos['gambar']; ?>" class="card-img-top" alt="...">
                        <div class="card-body">
                            <h5 class="card-title"><?php echo $row_bansos['judul']; ?></h5>
                            <?php
                            ...
                            echo "<p class='card-text'>$deskripsi_bansos ...</p>";
                            ?>
                            <?php
                            // Periksa apakah link kosong atau tidak valid
                            if (!empty($row_bansos['link']) && filter_var($row_bansos['link'], FILTER_VALIDATE_URL)) {
                            ?>
                                <a href="<?php echo $row_bansos['link']; ?>" class="btn btn-primary">Read More</a>
                            <?php } else { ?>
                                <a href="http://localhost/project/sosial/detail-artikel.php?id=<?php echo $row_bansos['id']; ?>" class="btn btn-primary">Read More</a>
                            <?php } ?>
                        </div>
                    </div>
                </div>
...
<!-- Display existing data for Kategori Keamanan -->
<div class="container-fluid existing-data">
    <div class="container">
        <h2 class="display-6 fw-bold">Data Artikel Kategori Keamanan</h2>
        <div class="row">
            <?php 
            // Fetch existing data from the database for Kategori Keamanan
            $query_keamanan = "SELECT * FROM artikel WHERE kategori = 'keamanan'";
            $result_keamanan = $koneksi->query($query_keamanan);
            while ($row_keamanan = $result_keamanan->fetch_assoc()) { ?>
                <div class="col-md-4 mb-4">
                    <div class="card">
                        <img src="<?php echo $row_keamanan['gambar']; ?>" class="card-img-top" alt="...">
                        <div class="card-body">
                            <h5 class="card-title"><?php echo $row_keamanan['judul']; ?></h5>
                            <?php
                            // Limit deskripsi maksimal 3 kalimat
                            $deskripsi_keamanan = $row_keamanan['deskripsi'];
                            $deskripsi_keamanan = implode(' ', array_slice(explode(' ', $deskripsi_keamanan), 0, 20));
                            echo "<p class='card-text'>$deskripsi_keamanan ...</p>";
                            ?>
                            <?php
                            // Periksa apakah link kosong atau tidak valid
                            if (!empty($row_keamanan['link']) && filter_var($row_keamanan['link'], FILTER_VALIDATE_URL)) {
                            ?>
                                <a href="<?php echo $row_keamanan['link']; ?>" class="btn btn-primary">Read More</a>
                            <?php } else { ?>
                                <a href="http://localhost/project/sosial/detail-artikel.php?id=<?php echo $row_keamanan['id']; ?>" class="btn btn-primary">Read More</a>
                            <?php } ?>
                        </div>
                    </div>
                </div>
      ...
      <div class="row text-center text-md-start">
        ...
        <div class="col-md-7 col-lg-8">
          <p>Copyright ©2022 All-Rights Reserved by:
            <strong class="text-warning">Della Fitria Lestari, Ninda, Irvianti Dwityara Sany</strong>
          </p>
        </div>
      </div>
    </div>
