...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['simpan'])) {
        ...
        if (!empty($jenis) && !empty($spesifikasi) && !empty($deskripsi)) {
            $sql = "INSERT INTO request (userId, nama, jenis, request, alasan)
                 values ('" . $username . "','" . $_SESSION['nama'] . "','" . $jenis . "','" . $spesifikasi . "','" . $deskripsi . "')";
...
if (isset($_POST['hapus'])) {
    ...
    $hapus = mysqli_query($koneksi, "DELETE FROM request WHERE id='$hapus_id'");
    if ($hapus) {
                ...
                <table class="table table-bordered table-striped">
                    ...
                    while ($tampil = mysqli_fetch_array($a)) : ?>
                        <tr>
                            ...
                            <td><?= $tampil['nama'] ?></td>
                            <td><?= $tampil['jenis'] ?></td>
                            <td><?= $tampil['request'] ?></td> 
                            <td><?= $tampil['alasan'] ?></td>                            
                            <td class="text-center">
                                <form action="" method="POST">
                                    <button type="submit" name="hapus" onclick="return confirm('Apakah yakin ingin menghapus data ini?')" class="btn btn-danger"><i class="fa-solid fa-trash"></i></button>
                                    <input type="hidden" name="hapus_id" value="<?= $tampil['id'] ?>">
                                </form>
                            </td>
                        </tr>
                    <?php endwhile; ?>
